...
// Mock users database with AdvancedPostData structure
...
// Saved posts with different authors
...
export default function SocialProfile() {
  ...
  const getPostsForTab = (): AdvancedPostData[] => {
    switch (activeTab) {
      case "saved":
        return savedPosts;
      case "liked":
  ...
  const currentUserInfo = {
    avatar: usersDatabase[currentLoggedInUser.username]?.avatar || "",
    name: usersDatabase[currentLoggedInUser.username]?.name || "Kullanıcı",
  };

  const postsToShow = getPostsForTab();
  ...
  return (
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          {/* Posts List using AdvancedPostCard */}
          {postsToShow.length === 0 ? (
            <div className="bg-card rounded-lg shadow-sm p-12 text-center">
              <p className="text-muted-foreground">{getEmptyMessageForTab()}</p>
            </div>
          ) : (
            <div className="space-y-4">
              {postsToShow.map((post) => (
                <AdvancedPostCard
                  ...
                  onBookmark={(postId, bookmarked) => console.log("Bookmark:", postId, bookmarked)}
